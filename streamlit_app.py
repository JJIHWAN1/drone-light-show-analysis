        ...
        if sns_df is not None:
            ...
            with sns_tab5:
                ...
                if len(youtube_data) > 0 and youtube_data['views'].sum() > 0:
                    ...
                    # 박스 플롯 설명
                    st.info("""
                    **📊 박스 플롯(Box Plot) 해석 가이드**
                    
                    박스 플롯은 데이터의 분포를 한눈에 보여주는 그래프입니다:
                    
                    - **상자(Box)**: 데이터의 중간 50% 범위 (25%~75%)
                    - **상자 안의 선**: 중앙값 (전체 데이터의 정중앙)
                    - **수염(Whisker)**: 정상 범위의 최솟값과 최댓값
                    - **점(●)**: 이상치 (극단적으로 높거나 낮은 값)
                    
                    💡 상자가 위에 있을수록 전반적으로 수치가 높고, 상자가 길수록 데이터 편차가 큽니다.
                    """)
