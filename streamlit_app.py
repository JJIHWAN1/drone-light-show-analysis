...
def main():
    ...
    if analysis_type == "🔍 네이버 검색 분석":
        ...
        with tab2:
            ...
            **왜 정규화가 필요한가?**  
            원본 데이터에서 부산·광안리는 고흥·녹동항보다 약 **19배** 많은 검색량을 기록합니다.  
            이 경우 부산 기준(0~100)으로 표시하면 고흥과 당진의 값은 모두 0~5 사이에 몰려 패턴을 확인할 수 없습니다.
            
            **정규화 방식:**  
            각 지역의 월별·계절별 **패턴을 비교**하기 위해, **각 지역별로 자체 최댓값을 100으로 정규화**했습니다.  
            - **고흥·녹동항**: 고흥의 최고 검색 시점 = 100 (원본: 1.44)
            - **당진·삽교호**: 당진의 최고 검색 시점 = 100 (원본: 3.45)
            - **부산·광안리**: 부산의 최고 검색 시점 = 100 (원본: 100.0)
            
            **차트 해석 방법:**  
            차트의 높이는 절대적 검색량이 아닌, **각 지역 내에서 해당 시기가 얼마나 성수기인지**를 나타냅니다.  
            예) 고흥 9월이 9.37 → 고흥의 최고점 대비 약 9% 수준 (고흥의 상대적 성수기)
            
            ⚠️ **주의:** 절대적 검색량은 부산이 압도적으로 많습니다. 이 차트는 각 지역의 **시간적 변화 패턴**을 비교하기 위한 것입니다.
            """)
