...
def main():
    ...
    if analysis_type == "🔍 네이버 검색 분석":
        ...
        with tab2:
            ...
            st.info("""
            **📊 검색 비율 계산 방법**  
            네이버 데이터랩에서 여러 지역을 동시에 비교할 때, **모든 지역을 통틀어 가장 높은 검색량을 100으로 설정**하고,  
            나머지는 그에 대한 상대적 비율(0~100)로 표현됩니다.  
            
            예) 부산·광안리의 최고 검색량이 100이라면, 고흥·녹동항의 최댓값이 1.5일 경우,  
            고흥은 부산 대비 약 1.5% 수준의 검색 관심도를 의미합니다.
            """)
